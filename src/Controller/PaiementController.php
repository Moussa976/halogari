...
{
    /**
     * Affiche le formulaire Stripe pour une reservation acceptee.
     *
    ...
    public function afficherFormulairePaiement(
        ...
        if ($reservation->getStatut() !== 'acceptee') {
            $this->addFlash('warning', 'Vous ne pouvez payer que si le conducteur a accepte votre reservation.');
            return $this->redirectToRoute('app_user_reservation', ['id' => $reservation->getId()]);
        }

        $stripePublicKey = $_ENV['STRIPE_PUBLIC_KEY'] ?? '';
        if ($stripePublicKey === '') {
            $this->addFlash('error', 'La configuration Stripe est incomplete. Le paiement est temporairement indisponible.');
            return $this->redirectToRoute('app_user_reservation', ['id' => $reservation->getId()]);
        }
        ...
        try {
            ...
            $this->addFlash('error', $e->getMessage() ?: 'Le paiement ne peut pas etre initialise pour le moment.');
            return $this->redirectToRoute('app_user_reservation', ['id' => $reservation->getId()]);
        }
    ...
    /**
     * Affiche la confirmation apres paiement capture.
     *
    ...
    public function confirmation(int $id, ReservationRepository $repo): Response
    {
        ...
        if (!$reservation || $reservation->getPassager() !== $this->getUser()) {
            throw $this->createNotFoundException('Acces interdit ou reservation introuvable.');
        }

        $paiement = $reservation->getPaiement();
        if (!$paiement || $paiement->getStatut() !== 'capture') {
            $this->addFlash('warning', "Le paiement n'est pas encore confirme. Si vous venez de payer, patientez quelques secondes puis actualisez vos reservations.");
            return $this->redirectToRoute('app_user_reservation', ['id' => $reservation->getId()]);
        }
